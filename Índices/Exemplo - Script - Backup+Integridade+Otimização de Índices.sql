    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          IF NOT EXISTS(SELECT * FROM @ActionsPreferred) SET @CurrentCommandSelect01 = @CurrentCommandSelect01 + ' AND ' + QUOTENAME(@CurrentDatabase) + '.sys.stats.stats_id IS NOT NULL'
        END
        ...
        EXECUTE sp_executesql @CurrentCommandSelect01
        SET @Error = @@ERROR
        IF @Error = 1222
        BEGIN
      ...
      BEGIN
        ...
        BEGIN
          IF @CurrentIsPartition = 0 SET @CurrentCommandSelect02 = 'IF EXISTS(SELECT * FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.objects ON ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[object_id] = ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[object_id] INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas ON ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[schema_id] = ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[schema_id] WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[type] IN(''U'',''V'') AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.is_ms_shipped = 0 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[type] IN(1,2,3,4) AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.is_disabled = 0 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.is_hypothetical = 0 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[schema_id] = @ParamSchemaID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[name] = @ParamSchemaName AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[object_id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[name] = @ParamObjectName AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[type] = @ParamObjectType AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.index_id = @ParamIndexID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[name] = @ParamIndexName AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[type] = @ParamIndexType) BEGIN SET @ParamIndexExists = 1 END ELSE BEGIN SET @ParamIndexExists = 0 END'
          IF @CurrentIsPartition = 1 SET @CurrentCommandSelect02 = 'IF EXISTS(SELECT * FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.objects ON ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[object_id] = ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[object_id] INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas ON ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[schema_id] = ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[schema_id] INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.partitions ON ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[object_id] = ' + QUOTENAME(@CurrentDatabase) + '.sys.partitions.[object_id] AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.index_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.partitions.index_id WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[type] IN(''U'',''V'') AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.is_ms_shipped = 0 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[type] IN(1,2,3,4) AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.is_disabled = 0 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.is_hypothetical = 0 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[schema_id] = @ParamSchemaID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[name] = @ParamSchemaName AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[object_id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[name] = @ParamObjectName AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[type] = @ParamObjectType AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.index_id = @ParamIndexID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[name] = @ParamIndexName AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[type] = @ParamIndexType AND ' + QUOTENAME(@CurrentDatabase) + '.sys.partitions.partition_id = @ParamPartitionID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.partitions.partition_number = @ParamPartitionNumber) BEGIN SET @ParamIndexExists = 1 END ELSE BEGIN SET @ParamIndexExists = 0 END'

          EXECUTE sp_executesql @CurrentCommandSelect02, N'@ParamSchemaID int, @ParamSchemaName sysname, @ParamObjectID int, @ParamObjectName sysname, @ParamObjectType sysname, @ParamIndexID int, @ParamIndexName sysname, @ParamIndexType int, @ParamPartitionID bigint, @ParamPartitionNumber int, @ParamIndexExists bit OUTPUT', @CurrentSchemaID, @CurrentSchemaName, @CurrentObjectID, @CurrentObjectName, @CurrentObjectType, @CurrentIndexID, @CurrentIndexName, @CurrentIndexType, @CurrentPartitionID, @CurrentPartitionNumber, @CurrentIndexExists OUTPUT
        ...
        BEGIN
          SET @CurrentCommandSelect03 = 'IF EXISTS(SELECT * FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.stats INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.objects ON ' + QUOTENAME(@CurrentDatabase) + '.sys.stats.[object_id] = ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[object_id] INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas ON ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[schema_id] = ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[schema_id] WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[type] IN(''U'',''V'') AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.is_ms_shipped = 0 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[schema_id] = @ParamSchemaID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.schemas.[name] = @ParamSchemaName AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[object_id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[name] = @ParamObjectName AND ' + QUOTENAME(@CurrentDatabase) + '.sys.objects.[type] = @ParamObjectType AND ' + QUOTENAME(@CurrentDatabase) + '.sys.stats.stats_id = @ParamStatisticsID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.stats.[name] = @ParamStatisticsName) BEGIN SET @ParamStatisticsExists = 1 END ELSE BEGIN SET @ParamStatisticsExists = 0 END'

          EXECUTE sp_executesql @CurrentCommandSelect03, N'@ParamSchemaID int, @ParamSchemaName sysname, @ParamObjectID int, @ParamObjectName sysname, @ParamObjectType sysname, @ParamStatisticsID int, @ParamStatisticsName sysname, @ParamStatisticsExists bit OUTPUT', @CurrentSchemaID, @CurrentSchemaName, @CurrentObjectID, @CurrentObjectName, @CurrentObjectType, @CurrentStatisticsID, @CurrentStatisticsName, @CurrentStatisticsExists OUTPUT
        ...
        BEGIN
          IF @CurrentIndexType = 1 SET @CurrentCommandSelect04 = 'IF EXISTS(SELECT * FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.columns INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.types ON ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.system_type_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.types.user_type_id OR (' + QUOTENAME(@CurrentDatabase) + '.sys.columns.user_type_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.types.user_type_id AND '+ QUOTENAME(@CurrentDatabase) + '.sys.types.is_assembly_type = 1) WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.[object_id] = @ParamObjectID AND (' + QUOTENAME(@CurrentDatabase) + '.sys.types.name IN(''xml'',''image'',''text'',''ntext'') OR (' + QUOTENAME(@CurrentDatabase) + '.sys.types.name IN(''varchar'',''nvarchar'',''varbinary'') AND ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.max_length = -1) OR (' + QUOTENAME(@CurrentDatabase) + '.sys.types.is_assembly_type = 1 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.max_length = -1))) BEGIN SET @ParamIsLOB = 1 END ELSE BEGIN SET @ParamIsLOB = 0 END'
          IF @CurrentIndexType = 2 SET @CurrentCommandSelect04 = 'IF EXISTS(SELECT * FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.index_columns INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.columns ON ' + QUOTENAME(@CurrentDatabase) + '.sys.index_columns.[object_id] = ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.[object_id] AND ' + QUOTENAME(@CurrentDatabase) + '.sys.index_columns.column_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.column_id INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.types ON ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.system_type_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.types.user_type_id OR (' + QUOTENAME(@CurrentDatabase) + '.sys.columns.user_type_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.types.user_type_id AND ' + QUOTENAME(@CurrentDatabase) + '.sys.types.is_assembly_type = 1) WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.index_columns.[object_id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.index_columns.index_id = @ParamIndexID AND (' + QUOTENAME(@CurrentDatabase) + '.sys.types.[name] IN(''xml'',''image'',''text'',''ntext'') OR (' + QUOTENAME(@CurrentDatabase) + '.sys.types.[name] IN(''varchar'',''nvarchar'',''varbinary'') AND ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.max_length = -1) OR (' + QUOTENAME(@CurrentDatabase) + '.sys.types.is_assembly_type = 1 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.columns.max_length = -1))) BEGIN SET @ParamIsLOB = 1 END ELSE BEGIN SET @ParamIsLOB = 0 END'

          EXECUTE sp_executesql @CurrentCommandSelect04, N'@ParamObjectID int, @ParamIndexID int, @ParamIsLOB bit OUTPUT', @CurrentObjectID, @CurrentIndexID, @CurrentIsLOB OUTPUT
        END

        -- Is Allow_Page_Locks set to On?
        IF @CurrentIndexID IS NOT NULL
        BEGIN
          SET @CurrentCommandSelect05 = 'IF EXISTS(SELECT * FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[object_id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[index_id] = @ParamIndexID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[allow_page_locks] = 1) BEGIN SET @ParamAllowPageLocks = 1 END ELSE BEGIN SET @ParamAllowPageLocks = 0 END'

          EXECUTE sp_executesql @CurrentCommandSelect05, N'@ParamObjectID int, @ParamIndexID int, @ParamAllowPageLocks bit OUTPUT', @CurrentObjectID, @CurrentIndexID, @CurrentAllowPageLocks OUTPUT
        END

        -- Is No_Recompute set to On?
        IF @CurrentStatisticsID IS NOT NULL
        BEGIN
          SET @CurrentCommandSelect06 = 'IF EXISTS(SELECT * FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.stats WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.stats.[object_id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.stats.[stats_id] = @ParamStatisticsID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.stats.[no_recompute] = 1) BEGIN SET @ParamNoRecompute = 1 END ELSE BEGIN SET @ParamNoRecompute = 0 END'

          EXECUTE sp_executesql @CurrentCommandSelect06, N'@ParamObjectID int, @ParamStatisticsID int, @ParamNoRecompute bit OUTPUT', @CurrentObjectID, @CurrentStatisticsID, @CurrentNoRecompute OUTPUT
        END

        -- Has the data in the statistics been modified since the statistics was last updated?
        IF @CurrentStatisticsID IS NOT NULL
        BEGIN
          SET @CurrentCommandSelect07 = 'IF EXISTS(SELECT * FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.sysindexes WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.sysindexes.[id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.sysindexes.[indid] = @ParamStatisticsID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.sysindexes.[rowmodctr] <> 0) BEGIN SET @ParamStatisticsModified = 1 END ELSE BEGIN SET @ParamStatisticsModified = 0 END'

          EXECUTE sp_executesql @CurrentCommandSelect07, N'@ParamObjectID int, @ParamStatisticsID int, @ParamStatisticsModified bit OUTPUT', @CurrentObjectID, @CurrentStatisticsID, @CurrentStatisticsModified OUTPUT
        END

        -- Is the index on a read-only filegroup?
        IF @CurrentIndexID IS NOT NULL
        BEGIN
          SET @CurrentCommandSelect08 = 'IF EXISTS(SELECT * FROM (SELECT ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.data_space_id FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.destination_data_spaces ON ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.data_space_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.destination_data_spaces.partition_scheme_id INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups ON ' + QUOTENAME(@CurrentDatabase) + '.sys.destination_data_spaces.data_space_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.data_space_id WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.is_read_only = 1 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[object_id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[index_id] = @ParamIndexID'
          IF @CurrentIsPartition = 1 SET @CurrentCommandSelect08 = @CurrentCommandSelect08 + ' AND ' + QUOTENAME(@CurrentDatabase) + '.sys.destination_data_spaces.destination_id = @ParamPartitionNumber'
          SET @CurrentCommandSelect08 = @CurrentCommandSelect08 + ' UNION SELECT ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.data_space_id FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups ON ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.data_space_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.data_space_id WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.is_read_only = 1 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[object_id] = @ParamObjectID AND ' + QUOTENAME(@CurrentDatabase) + '.sys.indexes.[index_id] = @ParamIndexID'
          IF @CurrentIndexType = 1 SET @CurrentCommandSelect08 = @CurrentCommandSelect08 + ' UNION SELECT ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.data_space_id FROM ' + QUOTENAME(@CurrentDatabase) + '.sys.tables INNER JOIN ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups ON ' + QUOTENAME(@CurrentDatabase) + '.sys.tables.lob_data_space_id = ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.data_space_id WHERE ' + QUOTENAME(@CurrentDatabase) + '.sys.filegroups.is_read_only = 1 AND ' + QUOTENAME(@CurrentDatabase) + '.sys.tables.[object_id] = @ParamObjectID'
          ...
          EXECUTE sp_executesql @CurrentCommandSelect08, N'@ParamObjectID int, @ParamIndexID int, @ParamPartitionNumber int, @ParamOnReadOnlyFileGroup bit OUTPUT', @CurrentObjectID, @CurrentIndexID, @CurrentPartitionNumber, @CurrentOnReadOnlyFileGroup OUTPUT
        END
